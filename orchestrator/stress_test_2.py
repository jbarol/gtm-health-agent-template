"""Stress Test 2: Board-ready diagnostic.

Open-ended diagnostic query that tests the agent's ability to decompose
a vague problem into new business / expansion / retention dimensions,
identify root causes, and produce actionable 90-day plan.
"""
...
STRESS_PROMPT = """\
Something's wrong with our business. Revenue is up 12% YoY but board says not growing fast enough, sales says pipeline weak, CS says customers unhappy. Figure out what's actually going on.

Decompose the problem:
- Is it new business, expansion, or retention?
- Who's underperforming and why?
- Is the pipeline actually weak or is it a conversion problem?
- Are customers actually unhappy or is it concentrated in a segment?

For each finding:
- Name specific people, segments, or cohorts with data
- Distinguish correlation from causation
- Tag whether it's a pipeline quality problem, execution problem, or market problem

Produce a 90-day plan for the board with:
- Concrete actions (not platitudes)
- Expected impact in dollars or points
- Owner suggestions (by role, not name)
- Timeline with milestones

Send findings to Slack as you discover them. Write the full board memo to /mnt/session/outputs/stress_test_2_board_memo.md.
...
print("=" * 60)
print("STRESS TEST 2: Board-Ready Diagnostic")
print("=" * 60)
...
session = client.beta.sessions.create(
    ...
    title="Stress Test 2: Board-Ready Diagnostic",
    vault_ids=vault_ids,
    resources=[
        {
            "type": "memory_store",
...
(output_dir / "stress_test_2_results.json").write_text(json.dumps(report, indent=2))

print("\n" + "=" * 60)
print("STRESS TEST 2 RESULTS")
print("=" * 60)
